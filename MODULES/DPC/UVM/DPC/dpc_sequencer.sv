class dpc_pkt extends uvm_sequence_item;
    ...
    `uvm_object_utils_begin(dpc_pkt)
        ...
        `uvm_field_int(is_dead, UVM_ALL_ON)
        `uvm_field_int(is_hot, UVM_ALL_ON)
...
class dpc_base_seq extends uvm_sequence#(dpc_pkt);
    `uvm_object_utils(dpc_base_seq)
    ...
    function new(string name = "dpc_base_seq");
        super.new(name);
    endfunction

    task do_reset();
        repeat(2) begin
            req = dpc_pkt::type_id::create("req");
            start_item(req);
...
class dpc_sequencer extends uvm_sequencer#(dpc_pkt);
    `uvm_component_utils(dpc_sequencer)
