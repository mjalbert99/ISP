class blc_pkt extends uvm_sequence_item;
    ...
    `uvm_object_utils_begin(blc_pkt)
...
class blc_base_seq extends uvm_sequence#(blc_pkt);
    `uvm_object_utils(blc_base_seq)
  
    localparam LINE_W = 16;
    localparam FRAME_W = 16;

    function new(string name = "blc_base_seq");
        super.new(name);
    endfunction

    task do_reset();
        repeat(2) begin
            req = blc_pkt::type_id::create("req");
            start_item(req);
...
class blc_sequencer extends uvm_sequencer#(blc_pkt);
    `uvm_component_utils(blc_sequencer)
