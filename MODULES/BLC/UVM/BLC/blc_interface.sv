interface blc_interface(input clk);
    ...
    valid_out_proc: assert property(@(posedge clk) disable iff(!resetn) valid_in |=> ## 2 valid_out)
        else `uvm_error("SVA_PROT_FAIL", "Valid_in to Valid_out not aligned0")

    not_valid_out_proc: assert property(@(posedge clk) disable iff(!resetn) !valid_in |=> ## 2 !valid_out)
