class gam_pkt extends uvm_sequence_item;
    ...
    `uvm_object_utils_begin(gam_pkt)
        ...
        `uvm_field_int(gam_red_in, UVM_ALL_ON)
        `uvm_field_int(gam_green_in, UVM_ALL_ON)
        `uvm_field_int(gam_blue_in, UVM_ALL_ON)
...
class gam_base_seq extends uvm_sequence#(gam_pkt);
    `uvm_object_utils(gam_base_seq)
  
    localparam LINE_W = 16;
    localparam FRAME_W = 16;

    function new(string name = "gam_base_seq");
        super.new(name);
    endfunction

    task do_reset();
        repeat(2) begin
            req = gam_pkt::type_id::create("req");
            start_item(req);
...
class gam_sequencer extends uvm_sequencer#(gam_pkt);
    `uvm_component_utils(gam_sequencer)
