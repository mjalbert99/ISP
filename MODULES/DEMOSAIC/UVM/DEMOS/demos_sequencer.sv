class demos_pkt extends uvm_sequence_item;
    ...
    `uvm_object_utils_begin(demos_pkt)
...
class demos_base_seq extends uvm_sequence#(demos_pkt);
    `uvm_object_utils(demos_base_seq)
  
    localparam LINE_W = 16;
    localparam FRAME_W = 16;

    function new(string name = "demos_base_seq");
        super.new(name);
    endfunction

    task do_reset();
        repeat(2) begin
            req = demos_pkt::type_id::create("req");
            start_item(req);
...
class demos_sequencer extends uvm_sequencer#(demos_pkt);
    `uvm_component_utils(demos_sequencer)
