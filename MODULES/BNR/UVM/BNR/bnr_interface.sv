interface bnr_interface(input clk);
    ...
    clocking cg_mon@(posedge clk);
        ...
        input is_green, is_edge;
        input wr_addr, rd_addr, x_count, y_count;
    endclocking

    valid_out_proc: assert property(@(posedge clk) disable iff(!resetn) valid_in && y_count >= 2 |=> ## 10 valid_out)
        else `uvm_error("SVA_PROT_FAIL", "Valid_in to Valid_out not aligned0")

    not_valid_out_proc: assert property(@(posedge clk) disable iff(!resetn) !valid_in && y_count >= 2 |=> ## 10 !valid_out)
