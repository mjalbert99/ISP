class lsc_pkt extends uvm_sequence_item;
    ...
    `uvm_object_utils_begin(lsc_pkt)
...
class lsc_base_seq extends uvm_sequence#(lsc_pkt);
    `uvm_object_utils(lsc_base_seq)
  
    localparam LINE_W = 16;
    localparam FRAME_W = 16;

    function new(string name = "lsc_base_seq");
        super.new(name);
    endfunction

    task do_reset();
        repeat(2) begin
            req = lsc_pkt::type_id::create("req");
            start_item(req);
...
class lsc_sequencer extends uvm_sequencer#(lsc_pkt);
    `uvm_component_utils(lsc_sequencer)
